...
class IWD_All_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	const TO_EMAIL = 'help@interiorwebdesign.com';
	...
	private function IndexInfo() {
		...
		$indexes = Mage::getModel ( 'iwdall/indexes' )->GetIndexesColection ();
		
		foreach ( $indexes as $item ) {
			$info = "MODE: {$item['mode']},<br />
			STATUS: {$item['status']},<br />
			UPDATE AT: {$item['ended_at']}";
	...
	public function sendEmail($body, $subject, $fromEmail, $fromName, $attachment) {
		...
		try {
			...
			Mage::getSingleton ( 'core/session' )->addError ( Mage::helper ( 'iwdall' )->__ ( 'Unable to send email.' ) );
		}
	}
